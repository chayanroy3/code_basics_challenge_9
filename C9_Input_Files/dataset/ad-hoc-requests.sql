...
SELECT 
    ...
    SUM(discounted.base_price * discounted.`quantity_sold(before_promo)`) AS "total_revenue(before_promotion)",
    SUM(discounted.discounted_price * discounted.`quantity_sold(after_promo)`) AS "total_revenue(after_promotion)"
FROM 
    discounted
INNER JOIN 
